...
export function getDiffArray(prev: HashedData[], curr:HashedData[]) { // O(n) w/o moves
  const newIndices = [];
  const newIndicesSet = new Set();
  const prevHashesToIndices : {[hash: string]: number} = prev.reduce((acc, val, i) => {
    acc[val.hash] = i;
    return acc;
  ...
  const newHashesToIndices: {[hash: string]: number} = curr.reduce((acc, val, i) => {
    acc[val.hash] = i;
    return acc;
  }, {} as {[hash: string]: number})
  ...
  for (let i = 0; i < moves.length; i++) {
    ...
    const sigma = Math.abs(currMove.to - currMove.from) / (currMove.to - currMove.from)
  ...
  const movedTraversed: number[] = moves.filter(({ from, to }) => from !== to).reduce((acc, curr) => { acc.push(curr.from); acc.push(curr.to); return acc; }, [] as number[])

  // moves are not working
  return ({
    newIndices, removedIndices, movesIndices: movedTraversed
  })
}
